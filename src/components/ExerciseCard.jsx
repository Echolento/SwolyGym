...
export default function ExerciseCard(props) {
    ...
    const [setsCompleted, setSetsCompleted] = useState(0); // Correct array destructuring
    ...
    return (
        <div className='p-4 rounded-md flex flex-col gap-4 bg-slate-950 sm:flex-wrap'>
            ...
            <div className='grid grid-cols-2 sm:grid-cols-4 sm:pace-items-center gap-2'>
                ...
                <button className='flex flex-col p-2 rounded border-[1.5px] duration-200 border-solid border-blue-900 hover:border-blue-600 w-full' onClick={handleSetIncrement}>
                    <h3 className='text-slate-400 text-sm capitalize'>Set Completed</h3>
                    <p className='font-medium'>{setsCompleted} / 5</p> {/* Displaying setsCompleted */}
                </button>
            </div>
        </div>
    );
